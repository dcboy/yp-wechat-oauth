...
class OAuth {
  ...
  async request(opts = {}) {
    ...
    var response;
    try {
      response = await (0, _requestPromise2.default)(opts);
    } catch (err) {
    ...
    if (response.errcode) {
      var err = new Error(response.errmsg);
  ...
  async getAccessToken(code) {
    ...
    var url = `https://api.weixin.qq.com/sns/oauth2/access_token?${_querystring2.default.stringify(info)}`;
  ...
  async getJscode2session(js_code) {
    ...
    const url = `https://api.weixin.qq.com/sns/jscode2session?${_querystring2.default.stringify(info)}`;
  ...
  async refreshAccessToken(refreshToken) {
    ...
    var url = `https://api.weixin.qq.com/sns/oauth2/refresh_token?${_querystring2.default.stringify(info)}`;
  ...
  _getUser(options, accessToken) {
    ...
    var url = `https://api.weixin.qq.com/sns/userinfo?${_querystring2.default.stringify(info)}`;
    return this.request({
      method: 'GET',
      uri: url,
      json: true
  ...
  async verifyToken(openid, accessToken) {
    ...
    var url = `https://api.weixin.qq.com/sns/auth?${_querystring2.default.stringify(info)}`;
    var data = await this.request({
      method: 'GET',
      uri: url,
      json: true
